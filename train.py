...
def load_from_json():
    json_path = os.path.join(DIR, r"model.json")
...
def save_to_json(weight, bias):
    ...
    with open(json_path, "w") as file:
        data = {"weight": weight.tolist(), "bias": bias.tolist()}
        json.dump(data, file)
...
def get_image():
    path = os.path.join(DIR, r"train_labels.csv")
    while True:
...
def create_batch(size=64):
    ...
    while True:
        ...
        for i in range(size):
            image = next(images)
            filename, label = (image["filename"], image["label"])
            filename = os.path.join(DIR, rf"train\{filename}")
        ...
        yield batch_array, label_array, filenames
...
def run():
    ...
    batch_num = 0
    while batch_num < 5000:
        batch_num += 1
        inputs, labels, filenames = next(batch_generator)
        ...
        if train == "y":
            ...
            correct_answer["text"] = labels[0]
            guess["text"] = answers[0]
            image2 = Image.open(filenames[0])
...
correct_answer = tk.Label(root, text=".")
correct_answer.grid(row=0, column=0, pady=80, padx=50)

guess = tk.Label(root, text="?")
guess.grid(row=0, column=1, pady=80, padx=50)

image = Image.open(r"train\0.png")
image = ImageTk.PhotoImage(image)

print(image)
image_label = tk.Label(root, image=image)
image_label.grid(row=0, column=2, pady=80, padx=50)

button_continue = tk.Button(root, text="NEXT", command=run)
button_continue.grid(row=1, column=0, columnspan=3)
root.mainloop()
